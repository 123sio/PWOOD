...
class SemiRotatedFCOSHeadH2RV2MCL(RotatedAnchorFreeHead):
    ...
    def pre_processing(self, logits, alone_angle=True):
        ...
        for img_id in range(batch_size):
            ...
            img_logits.append(torch.cat([
                x[img_id].permute(1, 2, 0).reshape(-1, self.cls_out_channels) for x in cls_scores
            ], dim=0))

            if alone_angle:
                #print('#########',bbox_preds[0][img_id].permute(1, 2, 0).reshape(-1, 4).size())
                #print(decode_angle_preds[0][img_id].size())
                img_logits.append(torch.cat([
    ...
    def decode_sigle(self, img_logits, featmap_sizes):

        bbox_preds = img_logits[1]

        points = self.prior_generator.grid_priors(featmap_sizes,
                                                  dtype=bbox_preds[0].dtype,
                                                  device=bbox_preds[0].device)
        flatten_points = torch.cat(points)
        de_bbox_preds = self.bbox_coder.decode(flatten_points, bbox_preds)
        de_logits = []
        de_logits.append(img_logits[0])
        de_logits.append(de_bbox_preds)
        de_logits.append(img_logits[2])
        de_logits.append(bbox_preds)
        # print(de_logits[1].size())
        return de_logits
    ...
    def forward_train(self,
        ...
        if get_data:
           logits = self(x)
           featmap_sizes = [featmap.size()[-2:] for featmap in logits[0]]
           img_logits_list = self.pre_processing(logits, alone_angle=True)
           if get_de_data:
               b = logits[0][0].size(0)
               de_logits_list = []
               for i in range(b):
                   de_logits = self.decode_sigle(img_logits_list[i],
                                                                 featmap_sizes)
                   de_logits_list.append(de_logits)

               return de_logits_list, featmap_sizes
           else:
               return img_logits_list, featmap_sizes
        else:
            outs = self(x)
    ...
    def loss(self,
        ...
        all_level_points = self.prior_generator.grid_priors(
            ...
            dtype=bbox_preds[0].dtype,
            device=bbox_preds[0].device)
        labels, bbox_targets, angle_targets, bid_targets, centerness_targets = self.get_targets(
        ...
        if len(pos_inds) > 0:
            ...
            #print('compacted',compacted_bid_targets)
            #print('count',bcnt)
            #print(bmsk)
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list):
        ...
        labels_list, bbox_targets_list, \
            angle_targets_list, bid_targets_list, centerness_targets_list = multi_apply(
