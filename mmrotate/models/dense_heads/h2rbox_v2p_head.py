...
class H2RBoxV2PHead(RotatedAnchorFreeHead):
    ...
    def nested_projection(self, pred, target):#将预测rbox投影到以gt为水平方向的外界水平框
        target_xy1 = target[..., 0:2] - target[..., 2:4] / 2 #target x1y1
        target_xy2 = target[..., 0:2] + target[..., 2:4] / 2
        target_projected = torch.cat((target_xy1, target_xy2), -1)# target转换成xyxy形式
        pred_xy = pred[..., 0:2]
        pred_wh = pred[..., 2:4]
        da = pred[..., 4] - target[..., 4]
    ...
    def loss(self,
        ...
        flatten_bid_targets = torch.cat(bid_targets)  #所有point对应的gt xywha形式
        ...
        if len(pos_inds) > 0:
            ...
            #bid中储存pos_bid_targets中所有唯一的元素，idx储存pos中每个位置对应的元素在bid中的索引
            ...
            _, bidx, bcnt = torch.unique(
                compacted_bid_targets.long(),  #转换为长整型，即强制取证，所以加0.5后的box会被认为是同一个
                return_inverse=True,
                return_counts=True)
            bmsk = bcnt[bidx] == 2
            ...
            pair_angle_preds = torch.empty(
                *bid.shape, pos_angle_preds.shape[-1],
                device=bid.device).index_reduce_(
                    ...
                    include_self=False)[bmsk].view(-1, 2,#ori/trs本来是列堆在一起的，强制分成两列用于区分
